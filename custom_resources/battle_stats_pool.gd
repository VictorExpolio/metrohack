...
#quizás debería meter uno más ya que yo tengo 4?
...
func _get_all_battles_for_tier(tier: int) -> Array[BattleStats]:
	#Aqui filtramos el array devolviendo uno más pequeñod e nuestro tier
	return pool.filter(
...
func _setup_weight_for_tier(tier: int) -> void:
	...
	#summamos todos los pesos DEL TIER
