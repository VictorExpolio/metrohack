...
func _ready() -> void:
	#test_scene_statushandler()
	pass

func apply_statuses_by_type(type: Status.Type) -> void:
	#estos van enlazados al event_bus -> por ejemplo en el caso de draw/discard cartas
	if type == Status.Type.EVENT_BASED:
		return
	#del array de todos los STATUS filtramos por TIPO
	var status_queue: Array[Status] = _get_all_statuses().filter(
		func(status: Status):
			return status.type == type
	)
	#cuando la COLA de un tipo se vacia APLICADOS
...
func test_scene_statushandler() -> void:
...
func add_status(status: Status) -> void:
	#stackable == INSTENSITY && DURATION
	var stackable := status.stack_type != Status.StackType.NONE
	#Si es un NUEVO status FRESH
	if not _has_status(status.id):
		...
		return
	#caso de mejore tipo nuestro BATTERY
