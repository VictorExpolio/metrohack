class_name CardTooltipPopup
extends Control

const CARD_MENU_UI_SCENE = preload("res://scenes/ui/card_menu_ui.tscn")

@export var background_color : Color = Color("413f4cc8")
@export var background_card_color : Color = Color("25242cc8")

@onready var background: ColorRect = $Background
@onready var background_card: ColorRect = $BackgroundCard

@onready var tooltip_card: CenterContainer = %TooltipCard
@onready var card_description: RichTextLabel = %CardDescription
@onready var card_title: Label = $VBoxContainer2/VBoxContainer/CardTitle

func _ready() -> void:
	for card: CardMenuUI in tooltip_card.get_children():
		card.queue_free()
	
	background.color = background_color
	background_card.color = background_card_color

func show_tooltip(card : Card) -> void:
	var new_card := CARD_MENU_UI_SCENE.instantiate() as CardMenuUI
	tooltip_card.add_child(new_card)
	new_card.card = card
	new_card.tooltip_requested.connect(hide_tooltip.unbind(1))
	card_description.text = card.tooltip_text
	card_title.text = card.id
	show()

func hide_tooltip() -> void:
	if not visible:
		return
	
	for card: CardMenuUI in  tooltip_card.get_children():
		card.queue_free()
	
	hide() 

func _on_gui_input(event: InputEvent) -> void:
	if event.is_action_pressed("left_mouse"):
		hide_tooltip()
