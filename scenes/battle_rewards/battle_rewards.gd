...
const MONEY_ICON := preload("res://art/money.png")
const MONEY_TEXT := "%s gold"
...
func add_money_reward(amount: int) -> void:
	...
	money_reward.reward_icon = MONEY_ICON
	money_reward.reward_text = MONEY_TEXT % amount
	money_reward.pressed.connect(_on_gold_reward_taken.bind(amount))
	rewards.add_child.call_deferred(money_reward)
	...
	for i in run_stats.card_rewards:
		...
		for rarity: Card.Rarity in card_rarity_weights:
			if card_rarity_weights[rarity] > roll:
				...
				var picked_car := _get_random_available_card(available_cards, rarity)
				card_reward_array.append(picked_car)
				available_cards.erase(picked_car)
				break
...
func _setup_card_chances() -> void:
	card_reward_total_weight = run_stats.common_weight + run_stats.uncommon_weight + \
